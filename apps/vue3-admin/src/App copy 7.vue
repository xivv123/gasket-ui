<template>
  <div class="upload-container">
    <div class="drag-area" @dragover.prevent @drop="handleFileDrop">
      <i class="el-icon-upload"></i>
      <span>拖拽文件到这里上传</span>
    </div>
    <gk-button @click="uploadFiles" class="upload-button" v-if="files.length">
      <i class="el-icon-upload"></i>
      <span>开始上传</span>
    </gk-button>
...
const handleFileDrop = (event: any) => {
  ...
  for (const file of event.dataTransfer.files) {
    ...
    files.value.push({
      ...
      fillDuration: parseFloat(estimatedTime),
      fillActive: false,
    });
  }
};
